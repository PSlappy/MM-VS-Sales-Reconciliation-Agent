...
This repo lives locally alongside sibling tools under a local `access-amenities-tools/` folder
(not itself a GitHub repo -- each tool gets its own; this one is
...
The user's decision as of 2026-09-09: **stay on this Mac for now**; a small always-on home
device and a cloud VM were both explicitly tabled for a later conversation, not declined. This
section exists so that future conversation doesn't have to be re-derived from scratch.

**Why this came up**: on 2026-09-09 the Mac was fully shut down (not asleep) overnight, and the
8 AM run never fired -- `launchd`'s calendar-interval catch-up only covers sleep, since
`launchd` itself isn't running during a real shutdown. Fixed for now with `RunAtLoad` (see
"How it runs" above), which covers "Mac was off, then turned back on" -- it does **not** cover
"Mac never gets turned on that day at all."

**Constraints that apply to any future direction**, all specific to this tool:

- **Credentials currently depend on macOS Keychain.** No macOS, no Keychain -- any non-Mac
  option needs a different secrets store (environment variables, a proper secrets manager, etc.)
- **MicroMart already blocks headless Chromium from this exact residential-IP setup.** This is
  the single biggest constraint on any hosting decision: a cloud VM adds a second red flag on
  top (datacenter IP ranges are exactly what anti-bot systems distrust most) and would likely
  make the login-blocking problem *worse*, not better, based on what's already been fought
  through in production. A small always-on device *on the same home network* avoids this,
  since it keeps the same residential IP reputation that's currently working.
- **Session persistence matters.** The "stays logged in most days, rarely needs a fresh
  TOTP-consuming login" design depends on a long-lived Playwright browser profile
  (`browser-state/`). Anything ephemeral (e.g. a fresh container per run, like typical CI/CD
  runners) loses that every single run, which would mean far more frequent logins and far more
  exposure to the headless-block problem.
- **The local file drop** (`~/Desktop/Access-Amenities/VendSoft/Sales-Import/...`) is
  Mac-specific. Any off-Mac option needs a decision about whether that local copy still matters
  once the Drive upload already serves as the durable copy.

**Options considered, not yet chosen between**:

1. **Small always-on device on the home network** (e.g. Raspberry Pi, cheap mini PC) — keeps
   the residential IP advantage, removes the single-point-of-failure of the daily-driver Mac
   needing to be on. Real work: migrate off Keychain, lose the "visible Chrome window on your
   screen" convenience during a headed escalation (would need remote access, e.g. VNC/SSH, to
   ever watch it live).
2. **Cloud VM** (AWS/DigitalOcean/etc.) — fully off-premises, but real risk of worsening the
   MicroMart bot-detection problem (see above), and needs a proper secrets manager plus a
   strategy for preserving the browser profile across runs (not just relying on ephemeral
   compute).
